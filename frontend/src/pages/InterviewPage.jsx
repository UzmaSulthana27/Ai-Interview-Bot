...
const getRoleIcon = (role) => {
  ...
  if (r.includes('mern')) return '🌐';
  if (r.includes('system design')) return '🏗️';
...
const roles = [
  ...
  { id: 14, name: 'MERN Developer',       icon: '🌐', tag: 'fullstack' },
  { id: 15, name: 'System Design',        icon: '🏗️', tag: 'backend'   },
  ...
  return (
    <div className="bg-white dark:bg-[#000000] min-h-screen transition-colors duration-300">
      ...
      <main className="pt-24 px-4 md:px-8 pb-24 md:pb-16 max-w-4xl mx-auto">
        ...
        <div className="h-2 w-full bg-slate-100 dark:bg-[#0a0a0a] rounded-full mb-4 overflow-hidden" style={isDark ? { background: '#0a1f0f', borderRadius: '4px' } : {}}>
          <motion.div 
            className="h-full bg-gradient-to-r from-[#2d5a27] to-[#1a3d16] dark:bg-none"
            style={isDark ? { background: '#00ffa3', boxShadow: '0 0 6px #00ffa3' } : {}}
            initial={{ width: 0 }}
          ...
          {loadingMsg && !feedback && !submitted ? (
            ...
            >
              ...
              <motion.span 
                ...
                style={isDark ? { background: '#00ffa3', boxShadow: '0 0 8px #00ffa3' } : { background: '#00ffa3' }}
        ...
        {format === 'MCQ' && mcqData && typedQuestion && (
          ...
          >
            {mcqData.options.map((opt) => {
              ...
              if (submitted) {
                if (opt.key === mcqData.correctAnswer) {
                  if (isDark) {
                    btnClass += " bg-[var(--bg-mcq-correct)] border-[var(--border-mcq-correct)] text-[var(--text-mcq-correct)] font-bold ";
                  } else {
                    btnClass = "w-full text-left p-[14px] px-4 rounded-lg font-mono mb-2 border-2 border-[#1D9E75] bg-[#f0fff4] text-[#1D9E75] font-bold ";
                  }
                } else if (opt.key === selectedOption && opt.key !== mcqData.correctAnswer) {
                  if (isDark) {
                    btnClass += " bg-[var(--bg-mcq-wrong)] border-[var(--border-mcq-wrong)] text-[var(--text-mcq-wrong)] font-bold ";
                  } else {
                    btnClass = "w-full text-left p-[14px] px-4 rounded-lg font-mono mb-2 border-2 border-[#cc0000] bg-[#fff5f5] text-[#cc0000] font-bold ";
                  }
                } else {
                  if (isDark) btnClass += " bg-[var(--bg-mcq-option)] border-[var(--border-mcq-default)] text-[var(--text-mcq-default)] opacity-50 ";
                ...
                if (selectedOption === opt.key) {
                  ...
                  if (isDark) {
                    btnClass += " bg-[var(--bg-mcq-option)] border-[var(--border-mcq-default)] text-[var(--text-mcq-default)] hover:border-[#00ffa3] hover:text-[#00ffa3] hover:translate-x-1 cursor-pointer ";
                  } else {
                    btnClass += " hover:border-[#888888] hover:translate-x-1 cursor-pointer ";
                  }
                }
              }
            ...
            {submitted && feedback && (
              <motion.div 
                className="mt-6 p-6 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#000000]"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
              >
                {isCorrect ? (
                ...
                <motion.button
                  ...
                  className="w-full mt-6 bg-slate-900 dark:bg-[#00ffa3] dark:text-[#000000] text-white py-4 rounded-xl font-bold shadow-lg flex items-center justify-center gap-2"
                  whileHover={{ scale: 1.01, y: -2 }}
                  whileTap={{ scale: 0.99 }}
                >
                  Next Question →
